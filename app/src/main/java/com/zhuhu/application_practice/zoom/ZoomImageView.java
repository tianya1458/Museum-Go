...
public class ZoomImageView extends ImageView implements OnGlobalLayoutListener,
    ...
    public interface OnScaleEndCallback {
        void onHide();

        void onShow();
    }

    private OnScaleEndCallback onScaleEndCallback;

    public void setOnScaleEndCallback(OnScaleEndCallback onScaleEndCallback) {
        this.onScaleEndCallback = onScaleEndCallback;
    }
...
//			scaleMatrix.postScale(scaleFactor, scaleFactor, getWidth()/2, getHeight()/2);
    ...
    private static int scaleHideCount = 0;
    ...
    public void onScaleEnd(ScaleGestureDetector detector) {
        if (onScaleEndCallback != null) {
            if (getDrawableScale() > initScale) {
                if (scaleHideCount == 0) {
                    onScaleEndCallback.onHide();
                    ++scaleHideCount;
                }
            } else {
                onScaleEndCallback.onShow();
                scaleHideCount = 0;
            }
        }
    }
